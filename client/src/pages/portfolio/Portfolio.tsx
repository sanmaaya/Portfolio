...
const Portfolio = () => {
   ...
   // Split other projects into DevOps and non-DevOps
   const devopsProjects = useMemo(() => {
      return otherProjects.filter(p =>
         p.tools_tech.some(t => ["Docker", "Jenkins", "AWS EC2", "GitHub Webhooks", "Nginx"].includes(t))
      );
   }, [otherProjects]);

   const nonDevopsOtherProjects = useMemo(() => {
      return otherProjects.filter(p =>
         !p.tools_tech.some(t => ["Docker", "Jenkins", "AWS EC2", "GitHub Webhooks", "Nginx"].includes(t))
      );
   }, [otherProjects]);
   ...
   const counts = useMemo<Record<string, number>>(() => {
      ...
      const d = devopsProjects.length;
      const o = nonDevopsOtherProjects.length;
      return {
         Featured: f,
         Community: c,
         Collab: x,
   ...
   const filteredProjects = useMemo(() => {
      ...
      const devops = devopsProjects.map((p) => ({
         ...p,
         category: "DevOps",
      }));
      const others = nonDevopsOtherProjects.map((p) => ({ ...p, category: "Others" }));
      ...
      else if (activeFilter === "DevOps") list = devops;
      else if (activeFilter === "Others") list = others;
      else list = [...featured, ...community, ...collab, ...devops, ...others];
